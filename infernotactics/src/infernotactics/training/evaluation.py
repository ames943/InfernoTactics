...
def eval_policy(policy, env, n_episodes=5, use_real_weather=True, deterministic=True,
    ...
    for ep in range(n_episodes):
        ...
        while not done:
            ...
            with torch.no_grad():
                if hasattr(policy, "decide_actions"):
                    tick_actions = policy.decide_actions(grid_np, scalars_np)
                else:
                    if hasattr(policy, '_forward') or hasattr(policy, 'forward'):
                        logits, _, _, zones, _features = _forward(policy, obs, env, device)
                    else:
                    ...
                    for _ in range(MAX_DISPATCH_SLOTS):
                        if hasattr(policy, '_forward') or hasattr(policy, 'forward'):
                            ...
                            if not bool(mask.any()):
                                break
                            r_idx = int(torch.argmax(resource_logits))
                            target_logits = action_logits.get("target", action_logits.get("zone"))
                            t_idx = int(torch.argmax(target_logits[0, r_idx]))
                            from infernotactics.policy.targets import resolve_relative_targets
    ...
    with torch.no_grad():
        for episode in range(episodes):
            ...
            while not done:
                ...
                tick_actions = []
                logits, _value, _classification, target_zones, _target_features = _forward(
                    model, obs, env, device
                )
                for _ in range(MAX_DISPATCH_SLOTS):
                    resource_logits = logits["resource_type"][0].clone()
