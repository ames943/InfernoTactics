"""Crash-safe CSV, JSONL, and TensorBoard logging for training runs."""
...
class RunLogger:
    def __init__(self, project_root, run_tag, config, trace_every=0, resume=False):
        ...
        if not resume or not os.path.exists(config_path):
            with open(config_path, "w", encoding="utf-8") as f:
                json.dump(manifest, f, indent=2, default=str)
        mode = "a" if resume else "w"
        episode_path = os.path.join(self.run_dir, "train_episode.csv")
        eval_path = os.path.join(self.run_dir, "eval.csv")
        checkpoint_path = os.path.join(self.run_dir, "checkpoints.csv")
        self.episode_file = open(episode_path, mode, newline="", encoding="utf-8")
        self.eval_file = open(eval_path, mode, newline="", encoding="utf-8")
        self.checkpoint_file = open(checkpoint_path, mode, newline="", encoding="utf-8")
        ...
        if not resume or os.path.getsize(episode_path) == 0:
            self.episode_writer.writeheader()
        if not resume or os.path.getsize(eval_path) == 0:
            self.eval_writer.writeheader()
        if not resume or os.path.getsize(checkpoint_path) == 0:
            self.checkpoint_writer.writeheader()
        trace_mode = "a" if resume else "w"
        self.trace_file = open(os.path.join(self.run_dir, "train_tick.jsonl"), trace_mode, encoding="utf-8") if self.trace_every else None
        self.writer = None
        try:
...
def summarize_episode(steps, episode, ignition, device, losses, seed=None):
