...
from infernotactics.policy.models.cnn_branch import CNNBranch, PER_CELL_CHANNELS, POOLED_DIM
from infernotactics.policy.models.classification_head import ClassificationHead
from infernotactics.policy.models.mlp_branch import MLPBranch, OUTPUT_DIM as MLP_OUTPUT_DIM
from infernotactics.policy.targets import NOOP_TARGET_INDEX, TARGET_FEATURE_DIM
from infernotactics.domain.observations import flatten_scalars
...
class RelativeInfernoModel(nn.Module):
    ...
    def forward(self, grid, scalars, target_zones, target_features):
        ...
        onehot_zones = onehot_zones * (target_zones >= 0).unsqueeze(-1)
        ...
        invalid_targets = target_zones < 0
        invalid_targets = invalid_targets.clone()
        invalid_targets[:, :, NOOP_TARGET_INDEX] = False
        target_logits = target_logits.masked_fill(invalid_targets, -1e9)
