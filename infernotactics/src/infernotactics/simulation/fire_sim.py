...
Runs directly on a validated static world grid. Each tick, every
...
from infernotactics.domain.fire import (
    BLAZE,
    BURNED_OUT,
    FUEL,
    SAFE,
...
from infernotactics.domain.weather import meteorological_wind_to_cartesian
from infernotactics.world.layers import LAYER_INDEX

# --- Layer indices in grid_static.npy (must match grid_builder.LAYER_NAMES) -
_L_ELEVATION = LAYER_INDEX["elevation"]
_L_SLOPE = LAYER_INDEX["slope"]
_L_BDENSITY = LAYER_INDEX["building_density"]
_L_BHEIGHT = LAYER_INDEX["building_height"]
_L_ROAD = LAYER_INDEX["road_mask"]
_L_FUEL = LAYER_INDEX["fuel_density"]
_L_WATER = LAYER_INDEX["water_mask"]
...
class FireSim:
    ...
    def step(self, wind_speed_mph=0.0, wind_direction_deg=0.0, humidity_pct=30.0):
        ...
        """
        wind_to_east, wind_to_north = meteorological_wind_to_cartesian(wind_direction_deg)
