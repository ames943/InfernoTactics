"""
Multi-ignition training, v2 -- three targeted fixes on top of
train_actor_critic_multi.py (kept untouched, alongside its 1000-episode
"before" logs/checkpoints, as the comparison baseline), after that run showed
1080 episodes of bistable oscillation with no sustained trend on either
validation point or stone_canyon. Root-cause diagnosis: the 4 scenarios this
policy has to handle (single_training + 3 multi-ignition points) have reward
scales differing by ~4 orders of magnitude (single_training ~+32 once solved,
stone_canyon ~-121k to -142k while unsolved) but shared ONE global return
normalizer and ONE scalar critic with no notion of which scenario it's in --
a recipe for gradient conflict between scenarios, not a sign the network can't
learn (v1 already proved it can solve individual scenarios in isolation,
repeatedly and reproducibly -- see mandeville_canyon's +33.1 hits).

Per the user's explicit instruction, this file does NOT touch: the reward
function, the environment/sim (inferno_env.py), the CNN/MLP architecture
SHAPES (CNNBranch/MLPBranch/ActorCritic/ClassificationHead internals are
unchanged -- only the MLP's INPUT WIDTH grows, via the existing n_scalars
constructor parameter, which is exactly what Fix 2 below asks for), or the
already-validated entropy bonus / split gradient clipping.

Fix 1 -- Per-scenario return normalization
  Replaced the single global RunningMeanStd with a dict, one instance per
  TRAINING_SCENARIO_NAMES entry (single_training, topanga_ridge,
  sullivan_canyon, stone_canyon). Each episode normalizes its returns using
  ONLY its own scenario's running stats before computing value loss/
  advantage, and updates only that scenario's stats afterward -- so
  stone_canyon's huge-magnitude, still-mostly-unsolved returns can no longer
  wash out single_training's small, already-solved-magnitude returns (or
  vice versa) in a single shared running mean/variance. All 4 normalizers'
  mean/std are printed every STATUS_EVERY episodes specifically so their
  divergence from each other is directly visible (proof the fix is engaged,
  not just present in the code).

Fix 2 -- Scenario identity as explicit MLP input
  A 5-slot one-hot (4 training scenarios + 1 "unseen" slot) is concatenated
  onto the standard flatten_scalars() vector before it reaches the MLP
  branch -- see build_scalars() below. This is deliberately NOT implemented
  by editing env.inferno_env.flatten_scalars() itself, since that function is
  shared by heuristic_policy.py, the single-scenario script, and eval.py's
  own untrained-model sanity check -- changing its output width would break
  all of those for no benefit to them. Instead, InfernoModel is constructed
  here with a larger n_scalars (its constructor already takes this as a
  parameter -- see models/inferno_model.py -- so this does NOT touch the
  model's architecture code, only how this script instantiates it), and
  src/train/eval.py gained one new OPTIONAL parameter (scalars_fn) so this
  script's eval calls can inject the one-hot without changing eval_policy()'s
  behavior for any caller that omits it.
  VALIDATION_IGNITION_POINTS (mandeville_canyon, getty_view_park) are NEVER
  trained on, so they don't get one of the 4 training slots -- giving them
  one would falsely tell the network "this is exactly scenario X" for a
  scenario it never actually is. An all-zero vector was considered but
  rejected: zero is also what a bug/missing-data path would produce, an
  ambiguous signal. Instead they get a dedicated 5th "unseen" slot -- an
  explicit, unambiguous "this is not one of your 4 training fires" signal,
  standard practice for held-out/out-of-distribution categorical encoding.

Fix 3 -- Curriculum init + curriculum sampling
  Model weights are warm-started from the COMPLETED single-scenario run's
  best.pt (models/checkpoints/best.pt, the one that beat the heuristic by
  +29,443 on its own training point) instead of random init. Only
  mlp.net.0.weight actually changes shape (its input width grew from
  len(SCALAR_KEYS)=8 to N_SCALARS=13 for Fix 2) -- its first 8 columns are
  copied from the old checkpoint, the new 5 (one-hot) columns are
  zero-initialized (neutral: the one-hot contributes nothing until training
  actually shapes it, rather than injecting arbitrary random noise into a
  brand-new input the old checkpoint never saw). Every other parameter
  (CNN, mlp.net.2/4, actor_critic, classifier) loads unchanged -- see
  build_model().
  curriculum_weights(episode) softens the transition from "mostly the
  already-solved scenario" to "genuinely multi-scenario": 80% single_training
  / 20% split evenly across the other 3 for episodes 1-300, linearly shifting
  to 50/50 by episode 1000, then uniform 25% each beyond ep 1000 (this run is
  only 1000 episodes -- see below -- so that last regime is defined for
  completeness/future extension but is not actually exercised here).

This run is 1000 episodes, not v1's 3500 -- v1 already produced 1080 episodes
of "no fixes" data as the comparison baseline; 1000 with these fixes is a
fair, time-bounded comparison point, not a fresh full run.

Everything else -- MPS-with-CPU-fallback, checkpoint/resume support (model +
optimizer + all 4 normalizers' state + ignition RNG state), the SIGTERM
handler re-raising as KeyboardInterrupt, graceful Ctrl+C handling -- is
carried over unchanged from train_actor_critic_multi.py; see that file's
docstring for the original rationale.

    python -m src.train.train_actor_critic_multi_v2                 # real 1000-episode run
    INFERNO_N_EPISODES=50 python -m src.train.train_actor_critic_multi_v2   # smoke test
"""
...
# --- Scenario identity (Fix 2/3) -----------------------------------------------
# Order fixes the one-hot index assignment (0-3) and the curriculum weight
# vector's order -- must stay consistent everywhere below.
...
UNSEEN_ONEHOT_INDEX = len(TRAINING_SCENARIO_NAMES)  # slot 4 -- validation scenarios, see module docstring
ONEHOT_DIM = len(TRAINING_SCENARIO_NAMES) + 1  # 5
N_SCALARS = len(SCALAR_KEYS) + ONEHOT_DIM  # 8 + 5 = 13, see module docstring Fix 2

SINGLE_SCENARIO_BEST_CKPT = os.path.join(PROJECT_ROOT, "models", "checkpoints", "best.pt")

# --- Hyperparameters -- IDENTICAL to train_actor_critic_multi.py except where the
# user's fixes explicitly call for a change (N_EPISODES, EVAL_EVERY) --------------
N_EPISODES = int(os.environ.get("INFERNO_N_EPISODES", 1000))  # not 3500 -- see module docstring
BASE_SEED = 2000
LEARNING_RATE = 3e-4
GAMMA = 0.99
...
STATUS_EVERY = 20
CHECKPOINT_EVERY = 20
EVAL_EVERY = 100  # was 20 in v1 -- explicitly requested cadence for v2
EVAL_EPISODES = 3

CHECKPOINT_DIR = os.path.join(PROJECT_ROOT, "models", "checkpoints_multi_v2")
LOG_DIR = os.path.join(PROJECT_ROOT, "logs")
TRAIN_LOG_PATH = os.path.join(LOG_DIR, "train_log_multi_v2.csv")
EVAL_LOG_PATH = os.path.join(LOG_DIR, "eval_log_multi_v2.csv")
...
# single_training's number is the ORIGINAL single-scenario run's measured heuristic
# baseline (same methodology: 5 episodes, seed=0, real weather, deterministic
# HeuristicPolicy) -- reused as-is, not remeasured, since nothing about that
# scenario or the heuristic changed. The other 5 are train_actor_critic_multi.py's
# per-individual-point baselines, also unchanged.
HEURISTIC_BASELINE = {
...
def curriculum_weights(episode):
    """Fix 3's schedule, as a pure function of episode number (1-indexed) --
    see module docstring. Returns a 4-vector of sampling probabilities in
    TRAINING_SCENARIO_NAMES order (single_training first)."""
    if episode <= 300:
        p_single = 0.80
    elif episode <= 1000:
...
def scenario_one_hot(scenario_name):
    """5-slot one-hot: index 0-3 for a training scenario, index 4 ("unseen")
    for anything else (i.e. a validation scenario name) -- see module
    docstring Fix 2."""
...
def build_scalars(obs_scalars, scenario_name):
    """flatten_scalars() (unchanged, from inferno_env.py) + this scenario's
    one-hot, concatenated -- the actual MLP input for this script. NOT a
    modification of flatten_scalars() itself -- see module docstring Fix 2."""
...
def build_model(n_grid_channels, device):
    """Fix 3's warm-start: random init, then overwrite every parameter except
    mlp.net.0.weight with the single-scenario run's best.pt exactly, and
    mlp.net.0.weight's first len(SCALAR_KEYS) columns with that checkpoint's
    values (its remaining ONEHOT_DIM columns -- an input the old checkpoint
    never had -- stay at their fresh zero-init). See module docstring."""
    model = InfernoModel(n_grid_channels=n_grid_channels, n_scalars=N_SCALARS).to(device)
    if not os.path.exists(SINGLE_SCENARIO_BEST_CKPT):
        print(f"[train-multi-v2] WARNING: {SINGLE_SCENARIO_BEST_CKPT} not found -- "
              f"falling back to random init instead of curriculum warm-start.")
        return model

    old_sd = torch.load(SINGLE_SCENARIO_BEST_CKPT, map_location=device, weights_only=False)
    new_sd = model.state_dict()
    old_n_scalars = len(SCALAR_KEYS)
    for key, old_value in old_sd.items():
        if key == "mlp.net.0.weight":
            merged = new_sd[key].clone()
            merged[:, :old_n_scalars] = old_value
            new_sd[key] = merged
        else:
            new_sd[key] = old_value
    model.load_state_dict(new_sd)
    print(f"[train-multi-v2] Warm-started from {SINGLE_SCENARIO_BEST_CKPT}: "
          f"mlp.net.0.weight columns 0-{old_n_scalars - 1} copied from checkpoint, "
          f"columns {old_n_scalars}-{N_SCALARS - 1} (scenario one-hot) zero-initialized; "
          f"all other layers loaded unchanged.")
    return model


class RunningMeanStd:
    """Welford's online mean/variance -- unchanged from v1, see that file's
    docstring. Fix 1 uses one instance PER training scenario instead of one
    shared instance -- see main()."""
...
def collect_rollout(env, model, ignition_point, scenario_name, device, seed):
    """Same rollout as v1's collect_rollout, except scalars are built via
    build_scalars() (flatten_scalars() + this episode's scenario one-hot,
    Fix 2) instead of plain flatten_scalars()."""
...
def update_policy(model, optimizer, steps, device, return_normalizer):
    """Identical to v1's update_policy -- `return_normalizer` here is the
    ONE instance for this episode's scenario (Fix 1), selected by the
    caller, not a shared global."""
...
def run_training_episode(env, model, optimizer, device, seed, return_normalizer, ignition_point, scenario_name):
    steps, total_reward, buildings_destroyed, contained = collect_rollout(
        env, model, ignition_point, scenario_name, device, seed
    )
...
def run_eval_suite(model, env, episode_num, eval_writer, eval_file, device):
    """6 scenarios individually -- the 4 training scenarios AND both held-out
    validation points -- each scored with model.forward() fed THIS scenario's
    one-hot (or the "unseen" slot for validation) via eval_policy()'s new
    scalars_fn hook (see src/train/eval.py). Returns {name: result}."""
...
def main():
    ...
    print(f"[train-multi-v2] {run_kind}: {N_EPISODES} episodes, initial device={device}")
    print(f"[train-multi-v2] Training scenarios: {TRAINING_SCENARIO_NAMES} "
          f"(curriculum-sampled, see curriculum_weights()); N_SCALARS={N_SCALARS} "
          f"(={len(SCALAR_KEYS)} base + {ONEHOT_DIM} one-hot)")

    print("[train-multi-v2] Building InfernoEnv...")
    ...
    if resuming:
        print(f"[train-multi-v2] Found {RESUME_STATE_PATH} -- resuming (not starting from episode 1).")
        ...
        print(f"[train-multi-v2] Resuming from episode {completed_episodes + 1}/{N_EPISODES} "
    ...
    if start_episode > N_EPISODES:
        print(f"[train-multi-v2] Resume state already shows {completed_episodes}/{N_EPISODES} episodes "
        ...
        try:
            for ep in range(start_episode, N_EPISODES + 1):
                ...
                try:
                    ...
                    print(f"[train-multi-v2] MPS op not implemented: '{op_name}' -- "
            ...
            interrupted = True
            print(f"\n[train-multi-v2] Caught interrupt (Ctrl+C or SIGTERM) after episode "
    ...
    print(f"\n=== Multi-ignition v2 (per-scenario norm + scenario one-hot + curriculum) summary ({label}) ===")
    ...
    print("Final per-scenario return normalizer stats (proof Fix 1 diverged them):")
    for name in TRAINING_SCENARIO_NAMES:
        rn = return_normalizers[name]
