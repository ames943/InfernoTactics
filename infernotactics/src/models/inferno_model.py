...
from models.cnn_branch import ADAPTIVE_POOL_SIZE, PER_CELL_CHANNELS, POOLED_DIM, CNNBranch  # noqa: E402
...
class InfernoModel(nn.Module):
    def __init__(self, n_grid_channels, n_scalars=len(SCALAR_KEYS),
        ...
        super().__init__()
        self.cnn = CNNBranch(in_channels=n_grid_channels, adaptive_pool_size=adaptive_pool_size)
    ...
    def forward(self, grid, scalars, value_head_idx=0):
        ...
        per_cell_features, pooled_cnn, zone_pooled = self.cnn(grid)
        mlp_features = self.mlp(scalars)
        fused = torch.cat([pooled_cnn, mlp_features], dim=1)
        action_logits, value = self.actor_critic(fused, zone_pooled, mlp_features, value_head_idx=value_head_idx)
